...
def test():
    ...
    if "glm" in args.model_name_or_path:
        ...
        prefix = "回答："
        inputs = tokenizer(prompt, [prefix + tokenizer.mask_token] * len(prompt),
                           truncation="only_first", return_token_type_ids=False, return_tensors="pt")
        max_gen_length = args.max_length - len(inputs['input_ids'][0])
        inputs_glm = tokenizer.build_inputs_for_generation(inputs, targets=target, max_gen_length=max_gen_length, padding=True)
