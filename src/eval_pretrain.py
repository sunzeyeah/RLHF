...
def test():
    ...
    prompt = [
        ...
        'X文件　　FPT文件DBF文件　　DIR文件这些文件是什么定义的？它们各自有什么特点与作用？\n回答：[MASK]'
        ]
    target = [
        '你机器里同时有国服和台服是不',#行的.那样会有冲突的.无法正常运行的.以下办法用下吧.台服客户端下载：  (1)打开CWOW文件夹，找到realmlist.wtf文件，用记事本打开它，先清空里面的内容，再添加“set realmlist  ”（不包括双引号），保存后退出，这样，在每次要进去台服的时候，只需要用WoW.exe来启动，如果是进去CWOW的话，就用Launcher.exe来启动，不会造成冲突。另外，用WoW.exe启动后，登陆的界面是CWOW的界面，这是因为使用的是CWOW的客户端的原因，不用怀疑，直接输入ID密码登陆后进入的就是台服。 ---------------------- (2)如果你已经有CWOW的客户端了，再想装TWOW的时候需要修改注册表。 修改方法： 1、\u3000点击\u3000开始 2、\u3000点击\u3000运行 3、\u3000在运行框里输入\u3000regedit4、\u3000在弹出的注册表框里点击\u3000编辑 5、\u3000点击查找 6、\u3000在查找框里输入\u3000Blizzard Entertainment 7、\u3000当左边框里搜索到\u3000Blizzard Entertainment\u3000的时候，在左边整个删除，一直搜索到出现对话框里说\u3000注册表搜索完毕\u3000为止 [安装] 然后是安装，直接解压，安装就OK了，安装后貌似是1.11.0的版本(视你下载的版本为定)。 --------------------- 补丁方面,直接通过程序下载,速度很快,一般能达到80K/S以上的速度(1M ADSL).---------------------------【注册账号】此处资料为转载，仅供参考------  直接申请体验CDK,然后开始录入个人资料. 召集令编号: RF070112000032 RF070112000033 RF070112000034 RF070112000035 RF070112000036 注：注册帐号时填写"召集令编号"可获得（14天）30小时体验优惠。 非必选项 录入个人资料这里,请注意一下,请尽量填写个人准确资料,如地区选(其它),以及真实身份证号码,真实住址,还有真实的email地址.(^^我选的住在钓鱼岛...不过地址栏下面的都写的真实的) 范例：南海诸岛钓鱼台列屿中华人民共和国江苏省南京市 ...XD~~ 至此,你的体验帐号基本申请完毕，可以去游戏里进行下体验和尝试了这样可以安装2个客户端,分别指向CWOW和TWOW.--------------------------【输入法】此处资料为转载，仅供参考----------五笔输入法的朋友，到   ,下载极点中文输入法，安装后在设置选项里调成输出繁体字。 拼音输入法的朋友，WINDOWS控制面板→区域和语言选项→语言→详细信息→添加→微软拼音输入法3.0，添加后在输入法的选项里调出“简/繁体”的按纽，在新增加的简繁体切换按纽按一下换成繁体即可. （推荐使用微软拼音输入法2003，效果比微软拼音输入法3.0要好，放出地址   ;下载后直接安装就可以了，会自动覆盖原来的微软拼音输入法3.0） -----------------------【充值月卡】-------------------转为正式帐号： 直接冲值月卡即可，不需要CDK，重大更新！~   ... _id=2&GoPage=1&lis= TWOW最大的优势就是月卡 插件方面,目前大部分的简体插件是能使用在TWOW的，您下载的魔兽精灵依然可以在TWOW使用。 最新补充 1/台湾没有距阵卡，注意帐号安全 2/注意保存密码，共有3种，分别为游戏密码，官网论坛密码，安全码， （本人好久没玩，安全码遗忘中，找客服已经解决） 3/台湾每周四早上4点例行维护，一般时间为6小时，大部分情况是提前开机',
    ...
    if "glm" in args.model_name_or_path:
        ...
        #                    max_length=args.max_length, padding="max_length", truncation="longest_first")
        inputs = tokenizer(prompt, return_tensors="pt", max_length=args.max_length-args.max_length_generation,
                           padding="max_length", truncation="longest_first")
        inputs_glm = tokenizer.build_inputs_for_generation(inputs, targets=target, max_gen_length=args.max_length_generation)
    ...
    if "glm" in args.model_name_or_path:
        ...
        inputs_glm = tokenizer.build_inputs_for_generation(inputs, max_gen_length=args.max_length_generation)
